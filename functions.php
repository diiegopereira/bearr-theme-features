...
function bearr_post_formats_scripts() {
	...
	//theme files	
	wp_enqueue_script( 'bearr-lightbox-js', plugin_dir_url( __FILE__ ) . '/js/custom/bearr-lightbox.js', array('jquery'), '20151215', true );
	wp_enqueue_script( 'bearr-post-formats-js', plugin_dir_url( __FILE__ ) . '/js/custom/bearr-post-formats.js', array('jquery'), '20151215', true );
...
//Transparent Header
function bearr_transparent_header( $meta_boxes ) {
    $prefix = 'bearr_'; 

    /*page subtitle*/
    $meta_boxes[] = array(
        'title'      => __( 'Header Style', 'bearr' ),
        'post_types' => 'page',
        'fields'     => array(
            array(
                'id'   => $prefix. 'header_background',
                'name' => __( 'Select the background style of the header', 'bearr' ),
                'type' => 'radio',
                'std'   => 'header-bg-default',
                'inline' => false,
                'options' => array(
                    'header-bg-default' => __( 'With Background Color', 'bearr' ),
                    'header-bg-transparent' => __( 'Transparent Background', 'bearr' ),
                ),
            ),        
        ),
    );
...
$bearr_setup_header_transparency = get_option( 'bearr_setup_header_transparency' );
if ( $bearr_setup_header_transparency == 'true' ) {
    add_filter( 'rwmb_meta_boxes', 'bearr_transparent_header' );
}