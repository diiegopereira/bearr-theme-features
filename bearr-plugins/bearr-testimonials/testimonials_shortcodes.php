...
function bearr_testimonial_shortcode($atts, $content = null) {
	...
	$retour .='<div class="testimonial-item"><blockquote>';
		...
		if (!empty($testimonial_image)) {
			$retour .='<div class="testimonial-image">';

				$retour .='<img src="'.$testimonial_image.'" alt="'.esc_html($testimonial_name).'"/>';
...
function bearr_testimonials_carousel_shortcode($atts, $content = null) {
	...
	if ( file_exists( get_template_directory() . '/framework/js/custom/custom-testimonials.js' ) ) {
		wp_enqueue_script( 'bearr-custom-testimonials-js', get_template_directory_uri() . '/framework/js/custom/custom-testimonials.js', array('jquery'), '20151215', true );
	}
	else {
