...
function bearr_gallery_item($atts, $content = null) {
	...
	wp_enqueue_script( 'simple-lightbox-js', plugin_dir_url( __FILE__ ) . 'js/vendor/simplelightbox/dist/simple-lightbox.min.js', array('jquery'), '20151215', true );
	if ( file_exists( get_template_directory() . '/framework/js/custom/custom-gallery.js' ) ) {
		wp_enqueue_script( 'bearr-custom-gallery-js', get_template_directory_uri() . '/framework/js/custom/custom-gallery.js', array('jquery'), '20151215', true );
	}
	else {
		wp_enqueue_script( 'bearr-custom-gallery-js', plugin_dir_url( __FILE__ ) . 'js/custom-gallery.js', array('jquery'), '20151215', true );
	}
	...
	foreach ( $gallery_images as $gallery_image ) {
	   ...
	   $retour .='<a href="'.$gallery_image_url_full .'" style="background-image: url('. $gallery_image_url .')">' ;

	   $retour .='<img src="'.$gallery_image_url .'" alt="Gallery Image">' ;
