...
function bearr_client_item($atts, $content = null) {
	...
	$retour .='<img src="'.$client_logo_url .'" alt="logo">' ;
	if (!empty($client_url)) {	
		$retour .='</a>' ;
	}
	$retour .='</div>';
...
function bearr_client_carousel($atts, $content = null) {
	...
	if ( file_exists( get_template_directory() . '/framework/js/custom/custom-clients.js' ) ) {
		wp_enqueue_script( 'bearr-custom-clients-carousel-js', get_template_directory_uri() . '/framework/js/custom/custom-clients.js', array('jquery'), '20151215', true );
	}
	else {
		wp_enqueue_script( 'bearr-custom-clients-carousel-js', plugin_dir_url( __FILE__ ) . 'js/custom-clients.js', array('jquery'), '20151215', true );
	}
